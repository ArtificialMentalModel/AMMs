...
def question_answer(prompt):
    ...
    #outputs = model.generate(input_ids=inputs["input_ids"].to('cuda'), top_p=0.9, temperature=0.9, attention_mask=inputs["attention_mask"], max_new_tokens=500, pad_token_id=tokenizer.eos_token_id)
    ...
    generated_tokens = outputs.sequences[0][inputs['input_ids'].size(1):]
...
output_csv = 'Synthetic_data_logits_llama3_8b_zeroshot.csv'
...
for sample in data:
    ...
    output_text, tokens = question_answer(prompt3)
    ...
    output_text = extract_entity_v2(output_text)
